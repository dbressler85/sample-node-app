...
async function pendingTrades(cookie, league) {
  ...
  try {
    ...
    // retry lives at the source now (mflRepo.pendingTrades), so a rate-limited read in the Home fan-out
    // recovers instead of swallowing the offer to [] and dropping the trade-offer triage card.
...
async function getDashboard(cookie, token, { deviceRosters = null } = {}) {
  ...
  // NOTE: no eager all-leagues divisionContext sweep here. It used to front-load a full `rosters` read
  // for every ≥2-division league (a 40-franchise read for a multi-copy league) into the SAME per-account
  // MFL queue as the reads this response is awaiting — and, being enqueued before the fan-out below, it
  // sat AHEAD of them (a priority inversion that added seconds to the cold Portfolio load). It's also
  // redundant now: every league's roster load below (`rosterFromDeviceFranchises` on the device path,
  // `getRoster`→`buildRoster` on the backend path) resolves divisionContext itself, so detection still
  // runs for every league — just inline, not front-loaded.
  ...
  // Arbitrage tolerates a partial load (unlike movers): it doesn't need history, and a 2-league spread
  // is still useful even if a third league didn't read. So we DON'T gate it on `partial` — that only
  // starved the section. What DID look "broken" was ghost rows: a player (or a league) whose name never
  // resolved rendered as a bare position badge + Shop with no name/value. We drop those unresolved
  // items at the source below, so a malformed row can never reach the app on ANY load state.
  const resolved = (s) => s && !/^Player \d+$/.test(s) && !/^Team \d+$/.test(s);
  const arbitrage = [...holdMap.values()]
    ...
    // Never emit a ghost row: the player AND both league legs must have resolved to real names (an
    // unresolved id renders as a bare badge + Shop with no name/value — the "broken" rows in the wild).
    .filter((a) => resolved(a.name) && resolved(a.high.name) && resolved(a.low.name))
