...
test('color law: value gold, neon accents, and the onAccent contrast fix are all present', () => {
  ...
  assert.equal(c.watch, '#D6F84E', 'watch is Neon Lime, distinct from value gold');
...
test('glow() builds the neon recipe: lit edge + interior wash + colored halo', () => {
  ...
  assert.equal(g.borderColor, 'rgba(214,248,78,0.55)', 'edge at 0.55');
  assert.equal(g.backgroundColor, 'rgba(214,248,78,0.12)', 'wash at 0.12');
  assert.equal(g.shadowColor, 'rgb(214,248,78)', 'halo tinted to the accent');
