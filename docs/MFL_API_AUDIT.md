...
| `tradeBait` (`tradebait.js`) | `WILL_GIVE_UP`,`IN_EXCHANGE_FOR`,`FRANCHISE` | ✅ **Correct** (overwrites prior bait, as intended). `tradeBait` has **no** `FRANCHISE_ID` param — the `FRANCHISE` we send is simply ignored. Form also documents a **256-char** `IN_EXCHANGE_FOR` cap we don't enforce (defensive-truncate is a nice-to-have, not wired). |
| `myWatchList` (per-league) | **intentionally NOT wired** | ⏸️ **Deliberate (2026-07-24).** MFL's `myWatchList` import (`L`+`ADD`/`REMOVE` CSVs) is a **per-league** watchlist. Our watchlist (`store/watchlist.js`) is a single **cross-league** list stored locally — one star, shown across every league with its per-league standing. That's the product surface; syncing one cross-league star to N per-league MFL lists mostly adds noise. Owner chose to **keep it local** — do not wire `myWatchList` without revisiting that decision. |
...
| cancel a queued claim (`waivers.js`) | **`blindBidWaiverRequest`/`waiverRequest`** `ROUND`+`REPLACE`+`PICKS=survivors` *(FIXED, #93)* | ✅ **FIXED** — was a local-store `remove` only, so an MFL-sourced claim (id `mfl-<system>-<round>-<idx>`) 404'd **and** the real bid still processed. MFL has no delete-one call; per the form, PICKS **append** to a round unless `REPLACE` is set, and "to clear the whole round, pass an empty value for PICKS." Cancel now re-reads the round from `pendingWaivers`, drops the target pick, and `REPLACE`s the round with the survivors (empty PICKS clears it) — so claims placed outside the app in the same round survive. Demo stays store-only. |
| make a pick → `draftPick` (`draft.js`) | **honest 501** *(to be replaced)* | 🔁 **Correction (2026-07-22, Misc reference):** a per-pick draft write **does** exist — it's the **Misc** command `live_draft` (`year/live_draft?CMD=DRAFT&PLAYER_PICK&ROUND&PICK`, `JSON=1`), **owner-accessible**, and `COMMENTS` is "meant for email drafts." It's not in the *Import* set, which is why the earlier pass missed it. Action: swap the 501 for a `live_draft?CMD=DRAFT` call (Misc URL shape, not `import?TYPE=`). Timer control (`PAUSE`/`RESUME`/`SKIP`/`UNDO`) is commissioner-only. The `draftResults`/`auctionResults` imports remain commissioner bulk-loads; `myDraftList` (owner) is the complementary auto-pick queue. See ROADMAP → "Owner writes unlocked…". |
